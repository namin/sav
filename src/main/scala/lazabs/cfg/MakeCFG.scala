...
object MakeCFG {
  ...
  def construct(o: Sobject): CFG = {
    ...
    CFG(start,trans,makeParentMap(trans),predMap,varsMap,Map.empty,Map.empty,Some(o),Map.empty)
  }
  def apply(o: Sobject, f: String, arrayRemoval: Boolean, accelerate: Boolean): CFG = {
    funcName = f
    val cfg = construct(o)
    ...
    case e :: rest => e match {
      ...
      case Assignment(lhs, rhs) =>
        ...
        val currentVariable: Variable = lhs match {
          ...
          case _ =>
            println("error in adding the variable")
            sys.exit(0)
        }
