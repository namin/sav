...
object Z3Wrapper {
  ...
  def toZ3Type(t: Type, z3: Z3Context): Z3Sort = t match {
    case IntegerType() | UnitType() => z3.mkIntSort
...
//    case SetType(lt)  => z3.mkSetSort(toZ3Type(lt, z3))
  ...
  def mkZ3Expr( e: Expression, z3: Z3Context): Z3AST = e match {
    ...
    case Implication(e1,e2) => z3.mkImplies(mkZ3Expr(e1, z3), mkZ3Expr(e2, z3))
...
//    case SetConst(elems) => elems.map(mkZ3Expr(_,z3)).foldLeft(z3.mkEmptySet(toZ3Type(IntegerType(),z3)))((a,b) => z3.mkSetAdd(a,b))
    ...
    case NumericalConst(num) => z3.mkInt(num, z3.mkIntSort)
    case BoolConst(v) if v => z3.mkTrue
    case BoolConst(v) if (!v) => z3.mkFalse
    case BinaryExpression(e1, op, e2) =>
      println("Error in Z3 conversion: missing binary op " + op)
      z3.mkFalse
    case _ =>
      println("Error in Z3 conversion: missing case " + e.getClass + "\n" + lazabs.viewer.ScalaPrinter(e))
      z3.mkFalse
  }
